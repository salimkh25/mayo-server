import { db, moveStock } from './db.js';

// Theme hero photography from the Stitch "Premium Juvenile Narrative" design system
const THEME_IMAGES: Record<string, string> = {
  school:
    'https://lh3.googleusercontent.com/aida-public/AB6AXuD3kOdhwyuizz331fWllSV4bdKeo5eXyZo15zkMIS-jAq1m5W2dDPHCU5tv0RsyMpUnxqq1eZinkDDRj8IeINCvUTZmfHXL2GpjGpVD-PZ-5Y1oDlJya9CDIPm98d9T0Z8HPmY9rp0fTiSIXo7BT3K6uzwfbWVVKfvDcN_FSxrxvNlWSjhg4RSOYiACgRY3TtBa4xVh6ukRVRkCZEpfg4nDBw3A9GV0OAa826IyXQPzj8XSInQNibufTwG2YTrLLKqgSUo1PtIuvRM',
  'pool-party':
    'https://lh3.googleusercontent.com/aida-public/AB6AXuD1Rmx5dCFLq3yT7Z-Rap2_WOM_CEcCzGywIRxUI2LfMjXucwF4gvtuhTXEVQF3yllZDbUmTI8Ma-p9hMpYRisjAXdTeaiCeCN5qQFqvkZ080QGpRixF5ytUH5T-eeQN5mjcZYFgNlxMrCZb6YpRDpXFime5fjBcbXvpHnRNnIjsi6eTdpYwTW2T_NlGC7CVDTN2hJKntbOoZYZvQwwUEcdB5ZbzES9pFZio6LG0B_ji-3-YkBtSLTeko0zoeB_CeVX4g2uwkEpr3A',
  'summer-vacation':
    'https://lh3.googleusercontent.com/aida-public/AB6AXuAHsbqOHnZZjcrYRm8n8dl0bcxuSgK5x3XAdUJMDUpA3VlKGvSQFwbv3JU9LjlwSb4-f1InYZ2axz4Ay6fnrVZd4QbSHUMsHCperuBXOuWse9WxHrx06LHqFDi8Ty7wG2CHQjd2M4KolkdhBczHCx1zsgAfcvx4NjkyZouP94Em777oYrgL2CZ8pG_oevh45_biteFdGbQTLrMbAVEcZ8HoC7r2fza7RBjzyu2bv0eIANBQS6dyEFMWvB0jinML5nCifHg1S4T488s',
  'eco-minimalist':
    'https://lh3.googleusercontent.com/aida-public/AB6AXuAQ56LgD7j0vHI1dVEfvOoI16oTbD5zVgkk7YCkv-jeCSDkHrpyQroKk1ZqhOcOsVqVt_nSkmpxfOnpGgErkwICK49P7j399aVTvXNrwdbwZEfefUJgLLAQoevn-ODG5obO41o21HpCpNr0neHFS_Iivj0cqizvHqliNbdRwhsGJUa4JXprtxPUg83mymkWYPpVoeyrmlfIDackEFis2lsRR8RxZM5GrAcHcOAM97dDHiHBQ3X0O0z1fWnINnPpZUYO6bNxemukoQ0',
};

export function backfillThemeImages() {
  const stmt = db.prepare(`UPDATE themes SET hero_image = ? WHERE slug = ? AND hero_image = ''`);
  for (const [slug, url] of Object.entries(THEME_IMAGES)) stmt.run(url, slug);
}

/** Give every item a standalone retail price (≈2.4× cost, ending in .90). */
export function backfillItemPrices() {
  const items = db.prepare(`SELECT id, cost_cents, list_price_cents FROM items`).all() as {
    id: number;
    cost_cents: number;
    list_price_cents: number;
  }[];
  const stmt = db.prepare(`UPDATE items SET list_price_cents = ? WHERE id = ?`);
  for (const it of items) {
    if (it.list_price_cents > 0) continue;
...
export function backfillPriceFloors() {
  const items = db.prepare(`SELECT id, cost_cents, list_price_cents, price_floor_cents FROM items`).all() as any[];
  const iStmt = db.prepare(`UPDATE items SET price_floor_cents = ? WHERE id = ?`);
  for (const it of items) {
    if (it.price_floor_cents > 0) continue;
    iStmt.run(Math.max(it.cost_cents, Math.round((it.list_price_cents * 0.8) / 10) * 10), it.id);
  }
  const outfits = db.prepare(`SELECT id, price_cents, price_floor_cents FROM outfits`).all() as any[];
  const oStmt = db.prepare(`UPDATE outfits SET price_floor_cents = ? WHERE id = ?`);
  for (const o of outfits) {
    if (o.price_floor_cents > 0) continue;
    oStmt.run(Math.round((o.price_cents * 0.8) / 10) * 10, o.id);
  }
}

export function seedIfEmpty() {
  const count = (db.prepare(`SELECT COUNT(*) AS n FROM themes`).get() as { n: number }).n;
  if (count > 0) return;
  ...
  const insTheme = db.prepare(
    `INSERT INTO themes (slug, name, tagline, description, icon, palette_from, palette_to, sort)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
  );
  const themes: Record<string, number> = {};
  const themeRows: [string, string, string, string, string, string, string, number][] = [
  ...
  ].map((r, i) => [...r, i] as [string, string, string, string, string, string, string, number]);
  for (const [slug, name, tagline, description, icon, from, to, sort] of themeRows) {
    ...
    themes[slug] = Number((db.prepare(`SELECT id FROM themes WHERE slug = ?`).get(slug) as any).id);
  }

  const insItem = db.prepare(
    `INSERT INTO items (sku, name, type, sizing, cost_cents, detail, icon, reorder_point)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
  );
  const items: Record<string, number> = {};
  // sku, name, type, sizing, cost, detail, icon, reorder point
  const itemRows: [string, string, string, string, number, string, string, number][] = [
  ...
  for (const [sku, name, type, sizing, cost, detail, icon, rp] of itemRows) {
    ...
    items[sku] = Number(db.prepare(`SELECT id FROM items WHERE sku = ?`).get(sku)!.id);
  }
  ...
  const oneSize: Record<string, number> = {
    'ACC-001': 18,
    'ACC-003': 3,
    'ACC-004': 16,
    'PRF-001': 24,
  ...
  const insOutfit = db.prepare(
    `INSERT INTO outfits (slug, name, story, price_cents, size_run, hero_image, palette_from, palette_to, icon, rating)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  );
  const insOI = db.prepare(`INSERT INTO outfit_items (outfit_id, item_id) VALUES (?, ?)`);
  const insOT = db.prepare(`INSERT INTO outfit_themes (outfit_id, theme_id) VALUES (?, ?)`);

  const sageImg =
    'https://lh3.googleusercontent.com/aida-public/AB6AXuDZ8cYT4RU7HWQwD_ma1EK5zcEj2FQBPYFI4x8ipqhjGn_bEMxp3H1lR1T7SKv1aDRv8Wi-OwIOysPfMdHMp_Af52S1oW6StwXXcI1bGLo0FyV6OuEz39ITEpcbsUVRDStwx09fXTy8xjG2VMTBO1cPThNtNG2ZHko_qM85nm8FVG2ojOTeD1UgDJnOrjQTzh1z1ioK4lY10QTeIEhaj614j_AQh4Msc_XEjHHZrS3waA5-YAlX-WHitTWeeqlRqF_OSO-zinsxfhQ';
  const poolImg =
  ...
  type OutfitSeed = [string, string, string, number, string[], string, string, string, string, number, string[], string[]];
  //    slug, name, story, price, sizes, hero, from, to, icon, rating, itemSkus, themeSlugs
  const outfitRows: OutfitSeed[] = [
  ...
  for (const [slug, name, story, price, sizes, hero, from, to, icon, rating, skus, themeSlugs] of outfitRows) {
    ...
    const oid = Number(db.prepare(`SELECT id FROM outfits WHERE slug = ?`).get(slug)!.id);
    for (const sku of skus) insOI.run(oid, items[sku]);
    for (const t of themeSlugs) insOT.run(oid, themes[t]);
  }

  // a few historical orders so the dashboard has numbers
  const insOrder = db.prepare(
    `INSERT INTO orders (email, customer_name, status, subtotal_cents, discount_cents, total_cents, tier_at_purchase, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now', ?))`
  );
  const insLine = db.prepare(
    `INSERT INTO order_lines (order_id, outfit_id, size, qty, unit_price_cents) VALUES (?, ?, ?, ?, ?)`
  );
  const demoOrders: [string, string, number, string, number, string][] = [
  ...
  for (const [email, cname, total, slug, qty, ago] of demoOrders) {
    ...
    const o = db.prepare(`SELECT id, price_cents FROM outfits WHERE slug = ?`).get(slug) as {
      id: number;
      price_cents: number;
    };
